---
title: "Classification for Weight Lifting Activity Recognition"
...
This Coursera Practical Machine Learning project report attempts to develop a classification model for supervised recognition of the quality of Weight lifting activity [Ref. 2]. The model is based on dataset from training 6 participants to lift barbells correctly and incorrectly in 5 different ways to constitute 5 classes for automatic classification. Based on known efficiencies, Random Forest method was chosen for training the classification model. The doParallel package was used on a multicore machine to siginificantly reduce the training time. The models from training were saved on disk for reloading subsequently for comparing classification errors of the variants of the random forest models. Random forest with inherent bootstrapping was compared with random forest with K-fold cross-validation (K=5 and K=10). The models do not appear to have significant differences in relation to confusion matrix and OOB errors. Other aspects of random forest, namely, variable importance, margin plots, tree size histograms, and outlier plots are used to examine the random forest model with ntree=1000. In addition, boosting method was also explored. Finally, the training models are used against test dataset to recognize the test classes. The RF and boosting models show identical classification decisions against the given 20 test vectors. However, it appears that RF with 10-fold cross-validation outperforms Boosting slightly in accuracy for this particular dataset. The 10-fold RF shows OOB error of 0.14% and accuracy of 0.9986240 for this particular dataset.
...
## Preprocessing of Weigtlifting Activity Dataset  
...
The plot above indicates that the classification error rate declines rapidly as the number of trees (ntree) exceeds 50. 
